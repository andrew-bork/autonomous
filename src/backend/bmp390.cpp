...
#include <backend/bmp390_macros.h>
...
double bmp390::compensate_temperature(std::uint32_t raw_temperature) {
    // double partial_data1;
    // double partial_data2;

    double partial_data1 = (double)(raw_temperature - coeffs.par_t1);
    double partial_data2 = (double)(partial_data1 * coeffs.par_t2);
...
static double pow_bmp3(const double base,uint8_t power) {
    double pow_output = 1;
    while (power != 0) {
...
double bmp390::compensate_pressure(double temp, std::uint32_t uncomp_pressure) {
    double t_lin = temp;
    ...
    partial_data4 = partial_data3 + pow_bmp3((double)uncomp_pressure, 3) * coeffs.par_p11;
...
bmp390::bmp390() : device(BMP390_ADDR){
    acquire_calib_vars();
}
    ...
    while(i < len) {
        ...
        if(frame_type == 0b10000000) { // sensor frame
            if(frame_param == 0b00010100) { 
                ...
                double temp = compensate_temperature(raw_temp);
                data[0] += temp;
...
void bmp390::acquire_calib_vars(){
    ...
    coeffs.par_t1 = ((double)reg_par_t1 / temp_var);
    reg_par_t2 = combine(reg_data[3], reg_data[2]);
    temp_var = 1073741824.0f;
    coeffs.par_t2 = ((double)reg_par_t2 / temp_var);
    reg_par_t3 = (int8_t)reg_data[4];
    temp_var = 281474976710656.0f;
    coeffs.par_t3 = ((double)reg_par_t3 / temp_var);
    reg_par_p1 = (int16_t)combine(reg_data[6], reg_data[5]);
    temp_var = 1048576.0f;
    coeffs.par_p1 = ((double)(reg_par_p1 - (16384)) / temp_var);
    reg_par_p2 = (int16_t)combine(reg_data[8], reg_data[7]);
    temp_var = 536870912.0f;
    coeffs.par_p2 = ((double)(reg_par_p2 - (16384)) / temp_var);
    reg_par_p3 = (int8_t)reg_data[9];
    temp_var = 4294967296.0f;
    coeffs.par_p3 = ((double)reg_par_p3 / temp_var);
    reg_par_p4 = (int8_t)reg_data[10];
    temp_var = 137438953472.0f;
    coeffs.par_p4 = ((double)reg_par_p4 / temp_var);
    reg_par_p5 = BMP3_CONCAT_BYTES(reg_data[12], reg_data[11]);

    /* 1 / 2^3 */
    temp_var = 0.125f;
    coeffs.par_p5 = ((double)reg_par_p5 / temp_var);
    reg_par_p6 = BMP3_CONCAT_BYTES(reg_data[14], reg_data[13]);
    temp_var = 64.0f;
    coeffs.par_p6 = ((double)reg_par_p6 / temp_var);
    reg_par_p7 = (int8_t)reg_data[15];
    temp_var = 256.0f;
    coeffs.par_p7 = ((double)reg_par_p7 / temp_var);
    reg_par_p8 = (int8_t)reg_data[16];
    temp_var = 32768.0f;
    coeffs.par_p8 = ((double)reg_par_p8 / temp_var);
    reg_par_p9 = (int16_t)combine(reg_data[18], reg_data[17]);
    temp_var = 281474976710656.0f;
    coeffs.par_p9 = ((double)reg_par_p9 / temp_var);
    reg_par_p10 = (int8_t)reg_data[19];
    temp_var = 281474976710656.0f;
    coeffs.par_p10 = ((double)reg_par_p10 / temp_var);
    reg_par_p11 = (int8_t)reg_data[20];
    temp_var = 36893488147419103232.0f;
    coeffs.par_p11 = ((double)reg_par_p11 / temp_var);
}
