...
struct mission_control {
     ...
     * @param t 
     */
    void bind_readable(std::string name, T* t);
...
template<typename T>
void mission_control::bind_readable(std::string name, T* t) {
...
void mission_control::bind_readable(std::string name, const T& t) {
    ...
    bound_readables.push_back([=]() -> std::string {
        // printf("%s\n", name.c_str());
        // auto t_ref = *t;
        return "\"" + name + "\":" + serialize::serialize(*t_ptr);
    });
}
